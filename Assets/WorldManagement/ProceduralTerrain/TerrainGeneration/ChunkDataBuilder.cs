...
namespace WorldManagement.TerrainGeneration
{
	...
	public class ChunkDataBuilder
	{
		...
		[Header("Components")]
		private TerrainChunkGenerator m_chunkGenerator;
		...
		public ChunkDataBuilder(TerrainChunkGenerator chunkGenerator, BiomeIndex biomeIndex)
		{
			m_chunkGenerator = chunkGenerator;
			m_biomeIndex = biomeIndex;

			chunkGenerator.StartCoroutine(BuildChunksFromQueue());
		}
		...
		public void DecorateChunk(TerrainChunk centerChunk)
		{
			...
			int worldSeed = WorldManager.s_Seed;
			Vector3Int chunkSize = WorldManager.s_ChunkSize;
		...
		private IEnumerator BuildChunksFromQueue()
		{
			while (m_isGenerationEnabled)
			{
				if (m_chunkQueue.Count > 0)
				{
					...
					yield return m_chunkGenerator.StartCoroutine(GenerateChunkBaseData(chunk.ChunkXZ, chunk.OnGenerationComplete));
				}
